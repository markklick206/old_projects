...
typedef struct tagBITMAPFILEHEADER {
	...
	UINT bfSize;		// size of the whole .bmp file
	WORD  bfReserved1;	// must be 0
	WORD  bfReserved2;	// must be 0
	UINT bfOffBits;	// offset, in bytes, from the BITMAPFILEHEADER structure to the bitmap bits
...
typedef struct tagBITMAPINFOHEADER {
	UINT biSize;			// size of the structure
	INT  biWidth;			// image width
	INT  biHeight;			// image Height
	WORD  biPlanes;			// bitplanes
	WORD  biBitCount;		// resolution
	UINT biCompression;	// compression
	UINT biSizeImage;		// size of the image
	INT  biXPelsPerMeter;	// pixels per meter X
	INT  biYPelsPerMeter;	// pixels per meter Y
	UINT biClrUsed;		// colors used
	UINT biClrImportant;	// important colors
...
private:
	...
	UINT bmp_size;   // size of the bmp_buffer array
